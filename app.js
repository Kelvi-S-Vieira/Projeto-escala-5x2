...
// ── Metas default (sobrescritas pelos inputs editáveis) ───────
...
// ── Produtividade por embalagem (Recebimento) — editável ──────
// Lar-Têxtil: CAIXA / FARDO / PACOTE   |   Vestuário: CABIDE / CAIXA
...
function initSemanaSelector() {
  ...
  const closest = all.reduce((a,b) => Math.abs(b-currentWeek)<Math.abs(a-currentWeek)?b:a);
  sel.value = closest;
}

/* ── Init metas editáveis ────────────────────────────────────── */
function initMetaInputs() {
  ["sep","rec","exp"].forEach(p => {
    const defaults = p==="sep" ? META_SEP_DEFAULT : p==="rec" ? META_REC_DEFAULT : META_EXP_DEFAULT;
    const setores  = p==="rec" ? SETORES_REC : SETORES_SEP;
    setores.forEach((s, i) => {
      const el = document.getElementById(`in-meta-${p}-${i}`);
      if (el) el.value = defaults[i].toLocaleString("pt-BR",{maximumFractionDigits:0});
    });
  });
}

/* ── Render data table (with editable meta) ──────────────────── */
...
function diffTable(id, setores, d61, d52) {
  ...
  document.getElementById(id).innerHTML = `
    ...
    <tbody>
      ${setores.map((s,i) => {
        const dm=d52[i].med-d61[i].med, dq=d52[i].qdr-d61[i].qdr;
        return `<tr>
          <td>${s}</td>
...
function renderCapTable(id, prefix, setores, meta, quadroDefault, prod, usaPack, hProd, pack) {
  ...
  const rows = setores.map((setor,i) => {
    const q=quadros[i], p=Array.isArray(pack)?(pack[i]||1):pack;
    const capDia=q*hProd*prod[i], volSem=capDia*5, pecas=usaPack[i]?volSem*p:volSem, diff=pecas-meta[i];
    return {setor,q,capDia,volSem,pecas,diff,i};
  });
  const totCap=rows.reduce((a,r)=>a+r.capDia,0), totVol=rows.reduce((a,r)=>a+r.volSem,0);
  const totPecas=rows.reduce((a,r)=>a+r.pecas,0), totMeta=meta.reduce((a,m)=>a+m,0);
  const totDiff=totPecas-totMeta, totQ=rows.reduce((a,r)=>a+r.q,0);
  const diffCell=(diff,cid)=>{ const cls=diff>0?"pos":diff<0?"neg":"",val=`${diff>0?"+":""}${fmt(diff)}`; return cid?`<td class="${cls}" id="${cid}">${val}</td>`:`<td class="${cls}">${val}</td>`; };
  if (!already) {
    document.getElementById(id).innerHTML=`
      <table><thead><tr>
        <th>Setor</th><th>Quadro</th><th>H. produtivas</th>
        <th>Cap. diária</th><th>Vol. semanal</th><th>Peças/sem</th><th>vs Meta</th>
      </tr></thead><tbody>
      ${rows.map(r=>`<tr>
        <td>${r.setor}</td>
        <td><input class="cap-input" id="cap-${prefix}-${r.i}" type="number" value="${r.q}" min="0" max="200" step="1" oninput="recalcCap()"></td>
      ...
      <tr>
        <td>Total</td><td id="cap-tot-q-${prefix}">${fmt(totQ)}</td><td>—</td>
  ...
  rows.forEach(r=>{
    ...
    dc.textContent=`${r.diff>0?"+":""}${fmt(r.diff)}`; dc.className=r.diff>0?"pos":r.diff<0?"neg":"";
  });
  ...
  td.textContent=`${totDiff>0?"+":""}${fmt(totDiff)}`; td.className=totDiff>0?"pos":totDiff<0?"neg":"";
}

/* ── Render capacity expedição (com veículos) ────────────────── */
function renderCapExpTable(id, prefix, setores, meta, quadroDefault, prod, usaPack, hProd, pack, pecasVeiculo) {
  const already = !!document.getElementById(`cap-${prefix}-0`);
  const quadros = setores.map((_,i)=>{ const el=document.getElementById(`cap-${prefix}-${i}`); return el?(parseInt(el.value)||0):quadroDefault[i]; });
  const rows = setores.map((setor,i)=>{
    const q=quadros[i], p=Array.isArray(pack)?(pack[i]||1):pack;
    const capDia=q*hProd*prod[i], volSem=capDia*5, pecas=usaPack[i]?volSem*p:volSem;
    const diff=pecas-meta[i], veic=pecasVeiculo>0?rup(pecas/5/pecasVeiculo):0;
    return {setor,q,capDia,volSem,pecas,diff,veic,i};
  });
  const totCap=rows.reduce((a,r)=>a+r.capDia,0), totVol=rows.reduce((a,r)=>a+r.volSem,0);
  const totPecas=rows.reduce((a,r)=>a+r.pecas,0), totMeta=meta.reduce((a,m)=>a+m,0);
  const totDiff=totPecas-totMeta, totQ=rows.reduce((a,r)=>a+r.q,0), totVeic=rows.reduce((a,r)=>a+r.veic,0);
  const diffCell=(diff,cid)=>{ const cls=diff>0?"pos":diff<0?"neg":"",val=`${diff>0?"+":""}${fmt(diff)}`; return cid?`<td class="${cls}" id="${cid}">${val}</td>`:`<td class="${cls}">${val}</td>`; };
  if (!already) {
    document.getElementById(id).innerHTML=`
      <table><thead><tr>
        <th>Setor</th><th>Quadro</th><th>H. produtivas</th>
        <th>Cap. diária</th><th>Vol. semanal</th><th>Peças/sem</th><th>vs Meta</th><th>Veículos/dia</th>
      </tr></thead><tbody>
      ${rows.map(r=>`<tr>
        <td>${r.setor}</td>
        <td><input class="cap-input" id="cap-${prefix}-${r.i}" type="number" value="${r.q}" min="0" max="200" step="1" oninput="recalcCap()"></td>
  ...
  rows.forEach(r=>{
    ...
    document.getElementById(`cap-vol-${prefix}-${r.i}`).textContent=fmt(r.volSem);
    document.getElementById(`cap-pec-${prefix}-${r.i}`).textContent=fmt(r.pecas);
    const dc=document.getElementById(`cap-diff-${prefix}-${r.i}`); dc.textContent=`${r.diff>0?"+":""}${fmt(r.diff)}`; dc.className=r.diff>0?"pos":r.diff<0?"neg":"";
  ...
  document.getElementById(`cap-tot-vol-${prefix}`).textContent=fmt(totVol);
  document.getElementById(`cap-tot-pec-${prefix}`).textContent=fmt(totPecas);
  const td=document.getElementById(`cap-tot-diff-${prefix}`); td.textContent=`${totDiff>0?"+":""}${fmt(totDiff)}`; td.className=totDiff>0?"pos":totDiff<0?"neg":"";
  document.getElementById(`cap-tot-veic-${prefix}`).textContent=totVeic>0?fmt(totVeic):"—";
}

/* ── Recalc only capacity ────────────────────────────────────── */
function recalcCap() {
...
function renderBars(hProd, pack) {
  ...
  const cap=SETORES_SEP.map((setor,i)=>{
    ...
    const p=Array.isArray(pack)?(pack[i]||1):pack;
    return {setor,q,pecas:q*hProd*PROD_SEP[i]*5*(USA_PACK_SEP[i]?p:1),color:colors[i]};
  });
  const maxP=Math.max(...cap.map(d=>d.pecas));
  document.getElementById("bars-sep").innerHTML=cap.map(d=>`
    <div class="bar-row">
      <div class="bar-label">${d.setor}</div>
      <div class="bar-track"><div class="bar-fill ${d.color}" style="width:${maxP>0?Math.round(d.pecas/maxP*100):0}%">${fmt(d.pecas)}</div></div>
      <div class="bar-ops">${d.q} op.</div>
    </div>`).join("");
}

/* ══════════════════════════════════════════════════════════════
   ALTERAÇÃO 2 — Escala de veículos por dia (Expedição)
   Média editável por dia da semana
   ══════════════════════════════════════════════════════════════ */
function renderVeicSched() {
  ...
  // Read edited values or derive from table average
  const veicBase = (() => {
    const totEl = document.getElementById("cap-tot-veic-exp");
    return totEl ? (parseInt(totEl.textContent.replace(/\D/g,""))||0) : 0;
  })();

  const cont = document.getElementById("veic-sched-grid");
  if (!cont) return;

  let html = `<div class="sched-cell sched-head" style="font-size:9px;color:var(--color-text-secondary,#7B8FA1)"></div>`;
  ...
  ["Equipe 1","Equipe 2"].forEach((eq,ei) => {
    ...
    (ei===0?eq1:eq2).forEach((v,di) => {
      if (!v) {
        const cls = di>=5?"sched-cell weekend":"sched-cell off-day";
        html += `<div class="${cls}">—</div>`;
      } else {
        const inputId = `veic-day-${ei}-${di}`;
        const existing = document.getElementById(inputId);
        const val = existing ? existing.value : (veicBase||"");
        html += `<div class="sched-cell on" style="padding:3px">
          <input class="veic-day-input" id="${inputId}" type="number"
            value="${val}" min="0" step="1" placeholder="Veíc"
            title="Veículos no dia — editável">
        </div>`;
      }
    });
  });
...
/* ══════════════════════════════════════════════════════════════
   ALTERAÇÃO 3 — Produtividade por embalagem + Quadro
   ══════════════════════════════════════════════════════════════ */
function getProdEmb() {
  const read = (id, def) => parseFloat(document.getElementById(id)?.value) || def;
  return {
    LAR:  {
      CAIXA:  read("prod-emb-lar-caixa",  PROD_EMB_DEFAULT.LAR.CAIXA),
      FARDO:  read("prod-emb-lar-fardo",   PROD_EMB_DEFAULT.LAR.FARDO),
      PACOTE: read("prod-emb-lar-pacote",  PROD_EMB_DEFAULT.LAR.PACOTE),
    },
    VEST: {
      CABIDE: read("prod-emb-vest-cabide", PROD_EMB_DEFAULT.VEST.CABIDE),
      CAIXA:  read("prod-emb-vest-caixa",  PROD_EMB_DEFAULT.VEST.CAIXA),
    },
  };
}

/* ── Render embalagem grid (cards resumo) ────────────────────── */
function renderEmbGrid(semana) {
  ...
  const card = (title, embs) => {
    ...
    const barItems=items.map((it,i)=>{const colors=["#2E5FA3","#F4A500","#0F6E56","#854F0B"];return`<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px"><div style="width:10px;height:10px;border-radius:2px;background:${colors[i]};flex-shrink:0"></div><span style="font-size:10px;color:#3D4A5C;flex:1">${it.label}</span><span style="font-size:11px;font-weight:700;color:#1B3A6B">${it.pct.toFixed(2)}%</span></div>`;}).join("");
    return `<div class="emb-card"><div class="emb-card-title">${title}</div><div class="emb-bar-wrap"><div style="display:flex;height:18px;border-radius:4px;overflow:hidden;width:100%">${strip(items)}</div></div><div style="margin-top:8px">${barItems}</div></div>`;
  };
...
/* ── Render embalagem breakdown com Quadro ───────────────────── */
function renderEmbBreakdown(semana) {
  const emb      = getEmbalagem(semana);
  const metaLar  = getMeta("rec",4)[0];
  const metaVest = getMeta("rec",4)[1];
  ...
  const hProd    = (parseFloat(document.getElementById("in-carga").value)||40)/5-1.8;
  const prodEmb  = getProdEmb();

  const EMB_COLORS = {CABIDE:"#2E5FA3",CAIXA:"#185FA5",FARDO:"#F4A500",PACOTE:"#0F6E56"};

  const mkRows = (embs, meta, pack, prodMap, dias52) => {
    ...
    return entries.map(([k,v],idx) => {
      ...
      const med52   = Math.ceil(volPack/dias52);
      ...
      return `<tr style="${bg}">
        <td style="text-align:left;padding:7px 10px;border-bottom:1px solid #EEF2F9">
          <span style="display:inline-block;width:9px;height:9px;border-radius:2px;background:${c};margin-right:7px;vertical-align:middle"></span>
        ...
        <td style="padding:7px 10px;text-align:right;border-bottom:1px solid #EEF2F9;${tc}">${qtdSem.toLocaleString("pt-BR")}</td>
        <td style="padding:7px 10px;text-align:right;border-bottom:1px solid #EEF2F9;${tc}">${volPack.toLocaleString("pt-BR")}</td>
        <td style="padding:7px 10px;text-align:right;border-bottom:1px solid #EEF2F9;font-weight:600;color:${isLast?"#F4A500":"#1B3A6B"}">${med52.toLocaleString("pt-BR")}</td>
        <td style="padding:7px 10px;text-align:right;border-bottom:1px solid #EEF2F9;font-weight:700;color:${isLast?"#F4A500":"#F4A500"}">${quadro}</td>
      </tr>`;
    }).join("");
  };

  const thead = `<thead><tr>
    <th style="text-align:left;padding:8px 10px;background:#1B3A6B;color:#fff;font-size:10px;letter-spacing:.5px">Embalagem</th>
    <th style="padding:8px 10px;text-align:right;background:#1B3A6B;color:#fff;font-size:10px">% Repr.</th>
    <th style="padding:8px 10px;text-align:right;background:#1B3A6B;color:#fff;font-size:10px">Qtd. peças</th>
    <th style="padding:8px 10px;text-align:right;background:#1B3A6B;color:#fff;font-size:10px">Vol. pack</th>
    <th style="padding:8px 10px;text-align:right;background:#1B3A6B;color:#fff;font-size:10px">Média/dia 5×2</th>
    <th style="padding:8px 10px;text-align:right;background:#1B3A6B;color:#fff;font-size:10px">Quadro</th>
  </tr></thead>`;

  const block = (title, embs, meta, pack, prodMap) => `
    <div>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:11px;overflow:hidden">
        ${thead}<tbody>${mkRows(embs,meta,pack,prodMap,5)}</tbody>
      </table>
    </div>`;

  document.getElementById("emb-breakdown").innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
      ${block("Lar-Têxtil", emb.lar,       metaLar,  packLar,  prodEmb.LAR)}
      ${block("Vestuário",  emb.vestuario, metaVest, packVest, prodEmb.VEST)}
    </div>`;
}
...
function renderKPIs(s61,s52,r61,r52,e61,e52) {
  ...
  const kpi=(label,v61,v52)=>{ const d=v52-v61; return `
    <div class="kpi"><div class="kpi-label">${label} 6×1</div><div class="kpi-val">${fmt(v61)}</div><div class="kpi-delta">operadores</div></div>
...
function renderProdPanel() {
  ...
  document.getElementById("prod-panel").innerHTML=`
    <div class="prod-grid" style="grid-template-columns:1fr 1fr 1fr">
      <div class="prod-block"><div class="prod-block-title">Separação — peças/operador/hora</div>${SETORES_SEP.map((s,i)=>bar(s,PROD_SEP[i],maxSep,i)).join("")}</div>
      <div class="prod-block"><div class="prod-block-title">Expedição — peças/operador/hora</div>${SETORES_EXP.map((s,i)=>bar(s,PROD_EXP[i],maxSep,i)).join("")}</div>
      <div class="prod-block"><div class="prod-block-title">Recebimento — peças/operador/hora</div>${SETORES_REC.map((s,i)=>bar(s,PROD_REC[i],maxRec,i)).join("")}</div>
    </div>`;
}
...
function recalc() {
  ...
  renderTable("tbl-rec-61",SETORES_REC,r61); renderTable("tbl-rec-52",SETORES_REC,r52);
